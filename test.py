...
def overall_unpleasantness_score():
    # Cost function we want to minimize is the total unpleasantness score of all tables in room 1 and (something about the
...
class TableArrange(Annealer):
    ...
    def move(self, T):
        ...
        randomRec = Rec(w, h, old.v1[0] + dx, old.v1[1] + dy, 0, 1)
...
for i in range(n):
    ...
    while not randomRec.valid():
        ...
        randomRec = Rec(w, h, random.randint(0, 19), random.randint(0, 10), 0, 1)
    state.append(randomRec)
    #print(randomRec.x, randomRec.y, randomRec.r)
